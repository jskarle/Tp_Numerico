...
contains
  ...
  subroutine biseccion ( f, a, b, n, tol, raiz, clave )
    ...
    ! METODO DE BISECCION para encontrar una soluci�n
    ! de f(x)=0 dada la funci�n continua f en el intervalo
    ! [a,b] donde f(a) y f(b) tienen signos opuestos.

    ! Argumentos
    procedure(func)          :: f     ! Funci�n que define la ecuaci�n
    real(wp), intent(in)     :: a     ! Extremo izquierdo del intervalo inicial
    real(wp), intent(in)     :: b     ! Extremo derecho del intervalo inicial
    integer,  intent(in out) :: n     ! L�mite de iteraciones/iteraciones realizadas
    real(wp), intent(in)     :: tol   ! Tolerancia para el error absoluto
    real(wp), intent(out)    :: raiz  ! Aproximaci�n a la raiz
  ...
  subroutine newton ( f, df, x0, n, tol, raiz, clave )
    ...
    ! soluci�n de f(x)=0 dada la funci�n derivable
    ! f y una aproximaci�n inicial x0.

    ! Argumentos
    procedure(func)          :: f     ! Funci�n que define la ecuaci�n
    procedure(func)          :: df    ! Derivada de la funci�n que define la ecuaci�n
    real(wp), intent(in)     :: x0    ! Aproximaci�n inicial a la ra�z
    integer,  intent(in out) :: n     ! L�mite de iteraciones/iteraciones realizadas
  ...
  subroutine secante ( f, x0, x1, n, tol, raiz, clave )
    ...
    ! ALGORITMO DE LA SECANTE para encontrar una soluci�n
    ! de f(x)=0, siendo f una funci�n continua, dada las
    ! aproximaciones iniciales x0 y x1.

    ! Argumentos
    procedure(func)          :: f     ! Funci�n que define la ecuaci�n
    real(wp), intent(in)     :: x0,x1 ! Aproximaciones iniciales a la ra�z
    integer,  intent(in out) :: n     ! L�mite de iteraciones/iteraciones realizadas
    real(wp), intent(in)     :: tol   ! Tolerancia para el error relativo
    real(wp), intent(out)    :: raiz  ! Aproximaci�n a la raiz
    integer,  intent(out)    :: clave ! Clave de �xito: 
  ...
  subroutine birge_vieta ( a, x0, n, tol, raiz, clave )
    ...
    ! El m�todo se basa en el m�todo de Newton-Raphson implementando el
    ! esquema de Horner para la evaluaci�n del polinomio y su derivada.
  ...
  subroutine punto_fijo ( f, x0, n, tol, raiz, clave )
    ...
    ! para encontrar una soluci�n de x=f(x) dada una
    ! aproximaci�n inicial x0.
